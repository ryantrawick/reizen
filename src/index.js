...
function UI() {
    ...
    this.controlTypeText = null
    this.cursorPointer = null
    this.cursorLine = null
    this.cursorLineMask = null
    this.cursorAngle = 0

    this.pausedStage = null

    this.controlArrowRight = null
    this.controlArrowLeft = null

    this.sensitivityArrowRight = null
    this.sensitivityArrowLeft = null
    this.sensitivityText = null

    this.horizontalArrowRight = null
    this.horizontalArrowLeft = null
    this.horizontalText = null

    this.verticalArrowRight = null
    this.verticalArrowLeft = null
    this.verticalText = null

    this.selectorState = 0 // 0 = control, 1 = sensitivity
    ...
    this.install = () => {
        ...
        this.pausedStage = new PIXI.Container()
    }
    ...
    this.start = () => {
        ...
        this.stage.addChild(this.progressTextShadow)
        ...
        this.stage.addChild(this.overheatTextShadow)
        this.overheatText = this.addTextGemma(320 / 2, 6, '16.23', 0xffffff, 0.5, 0.5)
        this.stage.addChild(this.overheatText)
        ...
        this.stage.addChild(this.scoreTextShadow)
        ...
        this.finalScoreTextShadow = this.addTextGemma(320 / 2 + 1, 240 / 2 + 78 + 1, 'Final Score: 8000', 0x000000, 0.5, 0.5)
        this.retryStage.addChild(this.finalScoreTextShadow)
        this.finalScoreText = this.addTextGemma(320 / 2, 240 / 2 + 78, 'Final Score: 8000', 0xffffff, 0.5, 0.5)
        this.retryStage.addChild(this.finalScoreText)
        ...
        this.controlTypeText = this.addTextGemma(320 / 2, 240 - 14, currentControlType, 0xffffff, 0.5, 0.5)
        this.stage.addChild(this.controlTypeText)

        this.sensitivityText = this.addTextGemma(320 / 2, 240 - 14 - 10, currentControlType, 0xffffff, 0.5, 0.5)
        this.stage.addChild(this.sensitivityText)

        this.controlArrowLeft = new PIXI.Sprite(this.loader.resources.arrow_left.texture)
        this.controlArrowLeft.position.x = 60
        this.controlArrowLeft.position.y = 240 - 14
        this.controlArrowLeft.anchor.set(0.5)
        //leftArrowControlType.interactive = true
        //leftArrowControlType.buttonMode = true
        // leftArrowControlType.on('pointerdown', (event) => {
        // 	if (currentControlType === ControlType.WASD) {
        // 		currentControlType = ControlType.MOUSE_POINTER
        // 	}
        // 	else if (currentControlType === ControlType.MOUSE_FULL) {
        // 		currentControlType = ControlType.WASD
        // 	}
        // 	else if (currentControlType === ControlType.MOUSE_POINTER) {
        // 		currentControlType = ControlType.MOUSE_FULL
        // 	}
        // 	this.controlTypeText.text = currentControlType
        // })
        this.stage.addChild(this.controlArrowLeft)

        this.controlArrowRight = new PIXI.Sprite(this.loader.resources.arrow_right.texture)
        this.controlArrowRight.position.x = 320 - 60
        this.controlArrowRight.position.y = 240 - 14
        this.controlArrowRight.anchor.set(0.5)
        //rightArrowControlType.interactive = true
        //rightArrowControlType.buttonMode = true
        // rightArrowControlType.on('pointerdown', (event) => {
        // 	if (currentControlType === ControlType.WASD) {
        // 		currentControlType = ControlType.MOUSE_FULL
        // 	}
        // 	else if (currentControlType === ControlType.MOUSE_FULL) {
        ...
        // 	this.controlTypeText.text = currentControlType
        // })
        this.stage.addChild(this.controlArrowRight)

        this.sensitivityArrowLeft = new PIXI.Sprite(this.loader.resources.arrow_left.texture)
        this.sensitivityArrowLeft.position.x = 60
        this.sensitivityArrowLeft.position.y = 240 - 14 - 10
        this.sensitivityArrowLeft.anchor.set(0.5)
        this.stage.addChild(this.sensitivityArrowLeft)

        this.sensitivityArrowRight = new PIXI.Sprite(this.loader.resources.arrow_right.texture)
        this.sensitivityArrowRight.position.x = 320 - 60
        this.sensitivityArrowRight.position.y = 240 - 14 - 10
        this.sensitivityArrowRight.anchor.set(0.5)
        this.stage.addChild(this.sensitivityArrowRight)

        this.verticalArrowLeft = new PIXI.Sprite(this.loader.resources.arrow_left.texture)
        this.verticalArrowLeft.position.x = 60
        this.verticalArrowLeft.position.y = 240 - 14 - 10 - 10
        this.verticalArrowLeft.anchor.set(0.5)
        this.stage.addChild(this.verticalArrowLeft)

        this.verticalArrowRight = new PIXI.Sprite(this.loader.resources.arrow_right.texture)
        this.verticalArrowRight.position.x = 320 - 60
        this.verticalArrowRight.position.y = 240 - 14 - 10 - 10
        this.verticalArrowRight.anchor.set(0.5)
        this.stage.addChild(this.verticalArrowRight)

        this.verticalText = this.addTextGemma(320 / 2, 240 - 14 - 10 - 10, currentControlType, 0xffffff, 0.5, 0.5)
        this.stage.addChild(this.verticalText)

        this.horizontalArrowLeft = new PIXI.Sprite(this.loader.resources.arrow_left.texture)
        this.horizontalArrowLeft.position.x = 60
        this.horizontalArrowLeft.position.y = 240 - 14 - 10 - 10 - 10
        this.horizontalArrowLeft.anchor.set(0.5)
        this.stage.addChild(this.horizontalArrowLeft)

        this.horizontalArrowRight = new PIXI.Sprite(this.loader.resources.arrow_right.texture)
        this.horizontalArrowRight.position.x = 320 - 60
        this.horizontalArrowRight.position.y = 240 - 14 - 10 - 10 - 10
        this.horizontalArrowRight.anchor.set(0.5)
        this.stage.addChild(this.horizontalArrowRight)

        this.horizontalText = this.addTextGemma(320 / 2, 240 - 14 - 10 - 10 - 10, currentControlType, 0xffffff, 0.5, 0.5)
        this.stage.addChild(this.horizontalText)

        this.cursorLine = new PIXI.Sprite(this.loader.resources.cursor_line.texture)
        this.cursorLine.visible = false
        this.cursorLine.anchor.set(0.0, 0.5)
        this.cursorLine.position.set(320 / 2, 240 / 2)
        this.stage.addChild(this.cursorLine)

        this.cursorPointer = new PIXI.Sprite(this.loader.resources.cursor_pointer.texture)
        this.cursorPointer.visible = false
        this.cursorPointer.anchor.set(0.5, 0.5)
        this.stage.addChild(this.cursorPointer)

        this.cursorLineMask = new PIXI.Graphics()
        this.cursorLine.mask = this.cursorLineMask

        this.pausedStage.addChild(this.addTextGemma(320 / 2 + 1, 240 / 2 + 1, 'PAUSED', 0x000000, 0.5, 0.5))
        this.pausedStage.addChild(this.addTextGemma(320 / 2, 240 / 2, 'PAUSED', 0xffffff, 0.5, 0.5))
    }
    ...
    this.render = () => {
        ...
        if (document.pointerLockElement !== game.renderer.domElement) {
            this.renderer.render(this.pausedStage)
            return
        }
        ...
        if (currentControlType === ControlType.MOUSE_FULL || currentControlType === ControlType.MOUSE_POINTER) {
            this.sensitivityText.visible = true
            this.sensitivityArrowLeft.visible = true
            this.sensitivityArrowRight.visible = true

            this.verticalText.visible = true
            this.verticalArrowLeft.visible = true
            this.verticalArrowRight.visible = true

            this.horizontalText.visible = true
            this.horizontalArrowLeft.visible = true
            this.horizontalArrowRight.visible = true

            this.sensitivityText.text = `Sensitivity: ${parseFloat(GLOBAL.mouseSensitivity).toFixed(2)}`

            this.verticalText.text = `Invert Y: ${(GLOBAL.mouseVerticalFlip ? 'true' : 'false')}`
            this.horizontalText.text = `Invert X: ${(GLOBAL.mouseHorizontalFlip ? 'true' : 'false')}`

            if (game.input.getButtonDown("selectUp") && this.selectorState < 3) {
                this.selectorState += 1
            }
            if (game.input.getButtonDown("selectDown") && this.selectorState > 0) {
                this.selectorState -= 1
            }
        } else {
            this.sensitivityText.visible = false
            this.sensitivityArrowLeft.visible = false
            this.sensitivityArrowRight.visible = false

            this.verticalText.visible = false
            this.verticalArrowLeft.visible = false
            this.verticalArrowRight.visible = false

            this.horizontalText.visible = false
            this.horizontalArrowLeft.visible = false
            this.horizontalArrowRight.visible = false
        }

        if (this.selectorState === 0) {
            this.controlArrowLeft.tint = rgbToHex(255, 255, 0)
            this.controlArrowRight.tint = rgbToHex(255, 255, 0)
            this.sensitivityArrowLeft.tint = rgbToHex(255, 255, 255)
            this.sensitivityArrowRight.tint = rgbToHex(255, 255, 255)
            this.verticalArrowLeft.tint = rgbToHex(255, 255, 255)
            this.verticalArrowRight.tint = rgbToHex(255, 255, 255)
            this.horizontalArrowLeft.tint = rgbToHex(255, 255, 255)
            this.horizontalArrowRight.tint = rgbToHex(255, 255, 255)
        } else if (this.selectorState === 1) {
            this.controlArrowLeft.tint = rgbToHex(255, 255, 255)
            this.controlArrowRight.tint = rgbToHex(255, 255, 255)
            this.sensitivityArrowLeft.tint = rgbToHex(255, 255, 0)
            this.sensitivityArrowRight.tint = rgbToHex(255, 255, 0)
            this.verticalArrowLeft.tint = rgbToHex(255, 255, 255)
            this.verticalArrowRight.tint = rgbToHex(255, 255, 255)
            this.horizontalArrowLeft.tint = rgbToHex(255, 255, 255)
            this.horizontalArrowRight.tint = rgbToHex(255, 255, 255)
        } else if (this.selectorState === 2) { // Vertical
            this.controlArrowLeft.tint = rgbToHex(255, 255, 255)
            this.controlArrowRight.tint = rgbToHex(255, 255, 255)
            this.sensitivityArrowLeft.tint = rgbToHex(255, 255, 255)
            this.sensitivityArrowRight.tint = rgbToHex(255, 255, 255)
            this.verticalArrowLeft.tint = rgbToHex(255, 255, 0)
            this.verticalArrowRight.tint = rgbToHex(255, 255, 0)
            this.horizontalArrowLeft.tint = rgbToHex(255, 255, 255)
            this.horizontalArrowRight.tint = rgbToHex(255, 255, 255)
        } else if (this.selectorState === 3) { // Horizontal
            this.controlArrowLeft.tint = rgbToHex(255, 255, 255)
            this.controlArrowRight.tint = rgbToHex(255, 255, 255)
            this.sensitivityArrowLeft.tint = rgbToHex(255, 255, 255)
            this.sensitivityArrowRight.tint = rgbToHex(255, 255, 255)
            this.verticalArrowLeft.tint = rgbToHex(255, 255, 255)
            this.verticalArrowRight.tint = rgbToHex(255, 255, 255)
            this.horizontalArrowLeft.tint = rgbToHex(255, 255, 0)
            this.horizontalArrowRight.tint = rgbToHex(255, 255, 0)
        }

        if (game.input.getButtonDown("selectRight")) {
            if (this.selectorState === 0) {
                if (currentControlType === ControlType.WASD) {
                    currentControlType = ControlType.MOUSE_FULL
                } else if (currentControlType === ControlType.MOUSE_FULL) {
                ...
                GLOBAL.mouseSensitivity = clamp(GLOBAL.mouseSensitivity + 0.05, 0.1, 8)
            } else if (this.selectorState === 2) {
                GLOBAL.mouseVerticalFlip = !GLOBAL.mouseVerticalFlip
            } else if (this.selectorState === 3) {
                GLOBAL.mouseHorizontalFlip = !GLOBAL.mouseHorizontalFlip
            }
        }

        if (game.input.getButtonDown("selectLeft")) {
            if (this.selectorState === 0) {
                if (currentControlType === ControlType.WASD) {
                    currentControlType = ControlType.MOUSE_POINTER
                } else if (currentControlType === ControlType.MOUSE_FULL) {
        ...
        if (currentControlType === ControlType.MOUSE_POINTER) {
            this.cursorPointer.visible = true
            this.cursorLine.visible = true

            this.cursorPointer.position.x += (game.input.states.lookX * (GLOBAL.mouseHorizontalFlip ? -1.0 : 1.0) * GLOBAL.mouseSensitivity) / (parseInt(game.renderer.domElement.style.maxHeight) / 240)
            this.cursorPointer.position.y += (game.input.states.lookY * (GLOBAL.mouseVerticalFlip ? -1.0 : 1.0) * GLOBAL.mouseSensitivity) / (parseInt(game.renderer.domElement.style.maxHeight) / 240)

            GLOBAL.pointerPosition.set(this.cursorPointer.position.x - (320 / 2), this.cursorPointer.position.y - (240 / 2))
            GLOBAL.pointerDirection.set(0, 0)
            GLOBAL.pointerDirection.sub(GLOBAL.pointerPosition)

            var xSub = 0 - (this.cursorPointer.position.x - (320 / 2));
            var ySub = 0 - (this.cursorPointer.position.y - (240 / 2));
            if (Math.sqrt(xSub * xSub + ySub * ySub) > (240 / 2)) {
                this.cursorPointer.position.x = (320 / 2) - GLOBAL.pointerDirection.normalize().x * (240 / 2)
                this.cursorPointer.position.y = (240 / 2) - GLOBAL.pointerDirection.normalize().y * (240 / 2)
            }

            this.cursorPointer.angle = ((Math.atan2(this.cursorPointer.position.y - (240 / 2), this.cursorPointer.position.x - (320 / 2)) + Math.PI) * (180 / Math.PI)) - 90
            this.cursorLine.angle = this.cursorPointer.angle - 90

            this.cursorLineMask.clear()
            this.cursorLineMask.beginFill()
            this.cursorLineMask.drawCircle(320 / 2, 240 / 2, Math.sqrt(Math.abs(this.cursorPointer.position.x - (320 / 2)) * Math.abs(this.cursorPointer.position.x - (320 / 2)) + Math.abs(this.cursorPointer.position.y - (240 / 2)) * Math.abs(this.cursorPointer.position.y - (240 / 2))))
            this.cursorLineMask.endFill()
        } else {
            this.cursorPointer.visible = false
            this.cursorLine.visible = false

            this.cursorPointer.position.x = 320 / 2
            this.cursorPointer.position.y = 240 / 2
        }
    ...
    this.ensureResources = (callback) => {
        ...
        this.loader
            ...
            .add('arrow_left', 'assets/arrow_left.png')
            .add('arrow_right', 'assets/arrow_right.png')
            .add('cursor_pointer', 'assets/cursor1.png')
            .add('cursor_line', 'assets/cursor_line1.png')
            .load(() => {
                callback()
            })
    }
...
function Board() {
    ...
    this.start = () => {
        ...
        this.particleMaterial = new THREE.RawShaderMaterial({
            ...
            depthTest: true, // TODO: Think about this
            depthWrite: true,
        ...
        this.musicSound.setMediaElementSource(document.getElementById('music'))
    ...
    this.setupStage = (x = 0, y = 0, z = 0, radius = 1) => {
        ...
        for (let i = 0; i < vertices.length; i += 3) {
            ...
            max = Math.max(new THREE.Vector3(vertices[i], vertices[i + 1], vertices[i + 2]).length(), max)
            min = Math.min(new THREE.Vector3(vertices[i], vertices[i + 1], vertices[i + 2]).length(), min)
...
function MagicGirlComponent(entity) {
    ...
    this.update = (delta) => {
        ...
        if (dead === false) {
            this.entity.transform.translateX(((game.input.getButton('left') ? -1 : 0) + (game.input.getButton('right') ? 1 : 0)) * (0.1 * (game.input.getButton('slow') ? 0.5 : 1.0)))
            this.entity.transform.translateY(((game.input.getButton('forward') ? -1 : 0) + (game.input.getButton('back') ? 1 : 0)) * (0.1 * (game.input.getButton('slow') ? 0.5 : 1.0)) * -1)
        }
...
function FallGameComponent(entity) {
    ...
    this.update = (delta) => {
        ...
        // TODO: Normalize here xxx~~~ NEVER
        // targetRotation = targetRotation * Quaternion.Euler(mouseInput.x, mouseInput.y, -Input.GetAxisRaw("Rotate") * currentSettings.rotateSpinSpeed * Time.deltaTime);
        if (dead === false) {
            if (currentControlType === ControlType.WASD) {
                this.rotation.multiply(this.newRotation.setFromEuler(
                    this.newRotationEuler.set(
                        ((((game.input.getButton('forward') ? -1.0 : 0.0) + (game.input.getButton('back') ? 1.0 : 0.0)) * (this.rotateSpeed * (game.input.getButton('slow') ? 0.55 : 1.0))) * (Math.PI / 180.0)) * delta,
                        ((((game.input.getButton('left') ? -1.0 : 0.0) + (game.input.getButton('right') ? 1.0 : 0.0)) * (this.rotateSpeed * (game.input.getButton('slow') ? 0.55 : 1.0))) * (Math.PI / 180.0)) * delta,
                        0
                    ))
                )
            } else if (currentControlType === ControlType.MOUSE_FULL) {
                this.rotation.multiply(this.newRotation.setFromEuler(
                    this.newRotationEuler.set(
                        ((game.input.states.lookY * (GLOBAL.mouseVerticalFlip ? -1.0 : 1.0) * GLOBAL.mouseSensitivity) * (Math.PI / 180.0)) * (game.input.getButton("slow") ? 0.333 : 1.0),
                        ((game.input.states.lookX * (GLOBAL.mouseHorizontalFlip ? -1.0 : 1.0) * GLOBAL.mouseSensitivity) * (Math.PI / 180.0)) * (game.input.getButton("slow") ? 0.333 : 1.0),
                        0
                    ))
                )
            } else if (currentControlType === ControlType.MOUSE_POINTER) {
                this.rotation.multiply(this.newRotation.setFromEuler(
                    this.newRotationEuler.set(
                        (-GLOBAL.pointerDirection.normalize().y * ((this.rotateSpeed * (game.input.getButton("slow") ? 0.75 : 1.5)) * easeInOutSine(THREE.inverseLerp(0, 240 / 2, GLOBAL.pointerDirection.set(0, 0).sub(GLOBAL.pointerPosition).length()))) * (Math.PI / 180.0)) * delta,
                        (-GLOBAL.pointerDirection.normalize().x * ((this.rotateSpeed * (game.input.getButton("slow") ? 0.75 : 1.5)) * easeInOutSine(THREE.inverseLerp(0, 240 / 2, GLOBAL.pointerDirection.set(0, 0).sub(GLOBAL.pointerPosition).length()))) * (Math.PI / 180.0)) * delta,
                        0
                    ))
                )
                //console.log(`x: ${GLOBAL.pointerDirection.normalize().x} y: ${GLOBAL.pointerDirection.normalize().y}\n
                //length: ${GLOBAL.pointerDirection.set(0, 0).sub(GLOBAL.pointerPosition).length()} speed: ${this.rotateSpeed * THREE.inverseLerp(0, 240, GLOBAL.pointerDirection.length())}`)
            }
        }
...
const ControlType = {
    WASD: 'WASD',
    MOUSE_FULL: 'Mouse Rotation',
    MOUSE_POINTER: 'Mouse Pointer'
}

let currentControlType = ControlType.WASD
