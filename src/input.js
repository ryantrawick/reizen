function Input() {
	...
	this.actions = {
		...
		lookX: { type: 'axis' },
		lookY: { type: 'axis' },
		selectRight: { type: 'button' },
		selectLeft: { type: 'button' },
		selectUp: { type: 'button' },
		selectDown: { type: 'button' },
	}
	...
	//     this.mouse.lastMove.x = event.movementX || 0
	//     this.mouse.lastMove.y = event.movementY || 0
	//     this.mouse.moved.x = true
	//     this.mouse.moved.y = true
	//   }
	this.mouseStates = { mouseX: { lastMove: 0, moved: false }, mouseY: { lastMove: 0, moved: false } }
	this.mouseActionMapping = {
		slow: [0],
		lookX: ['mouseX'],
		lookY: ['mouseY']
	}
	this.onMouseDown = (event) => {
		if (!this.mouseStates[event.button]) {
			this.mouseStates[event.button] = { down: false, held: false, up: false, checked: false }
		}
	}
	this.onMouseUp = (event) => {
		if (this.mouseStates[event.button]) {
			this.mouseStates[event.button].up = true
		}
	}
	this.onMouseMove = (event) => {
		this.mouseStates.mouseX.lastMove = event.movementX || 0
		this.mouseStates.mouseY.lastMove = event.movementY || 0
		this.mouseStates.mouseX.moved = true
		this.mouseStates.mouseY.moved = true
	}
	...
	this.update = () => {
		...
		// Update mouse state
		// if (this.mouse.moved.x === false) { //  || document.pointerLockElement !== renderer.domElement // TODO: Always get input, do pointer check in game code, when consumed
		//   this.mouse.delta.x = 0
		//   this.mouse.lastMove.x = 0
		//   this.mouse.moved.x = false
		// } else {
		//   this.mouse.delta.x = this.mouse.lastMove.x
		//   this.mouse.moved.x = false
		// }

		// if (this.mouse.moved.y === false) { //  || document.pointerLockElement !== renderer.domElement // TODO: Always get input, do pointer check in game code, when consumed
		//   this.mouse.delta.y = 0
		//   this.mouse.lastMove.y = 0
		//   this.mouse.moved.y = false
		// } else {
		//   this.mouse.delta.y = this.mouse.lastMove.y
		//   this.mouse.moved.y = false
		// }

		Object.keys(this.mouseStates).forEach(button => {
			if (button === 'mouseX' || button === 'mouseY') {
				return
			}

			if (!this.mouseStates[button].checked) {
				this.mouseStates[button].down = true
				this.mouseStates[button].held = true
				this.mouseStates[button].checked = true
			} else {
				this.mouseStates[button].down = false
			}

			if (!this.mouseStates[button].held) {
				delete this.mouseStates[button]
				return
			}

			if (this.mouseStates[button].up) {
				this.mouseStates[button].held = false
			}
		})

		// Send mouse state to main state
		Object.keys(this.mouseActionMapping).forEach(action => {
			if (!this.actions[action]) {
				console.error(`InputState is missing the '${action}' action`)
			}
			const type = this.actions[action].type
			...
			if (type === 'button') {
				...
				for (let i = 0; i < this.mouseActionMapping[action].length; i++) {
					const button = this.mouseActionMapping[action][i]

					if (!this.mouseStates[button]) {
						continue
					}

					pollDown += this.mouseStates[button].down ? 1 : 0
					pollHeld += this.mouseStates[button].held ? 1 : 0
					pollUp += this.mouseStates[button].up ? 1 : 0
				}
				...
				const axis = this.mouseActionMapping[action][0] // Only one bind allowed for an axis right now, makes sense I guess

				if (this.mouseStates[axis].moved === false) { // document.pointerLockElement !== renderer.domElement TODO: Should only be checked in game loop, not input
					this.states[action] = 0
					this.mouseStates[axis].lastMove = 0
					this.mouseStates[axis].moved = false
					return
				}

				if (this.states[action] === null) {
					this.states[action] = 0
				}

				// input.states[action] = 0 // Move this to a different input clearing system before this

				this.states[action] = this.mouseStates[axis].lastMove
				this.mouseStates[axis].moved = false
			}
		})
